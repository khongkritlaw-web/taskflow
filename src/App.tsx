...
import { SettingsModal, SETTINGS_TOPICS } from './components/SettingsModal';
...
export default function App() {
  ...
  const [activeSettingsModalTopic, setActiveSettingsModalTopic] = useState<string | null>(null);
            ...
            {activeTab === 'settings' && (
              ...
              >
                {!isSettingsUnlocked ? (
                  ...
                  <div className="space-y-6 pb-24 relative" id="unlocked-settings-container">
                    {/* STATUS CLOUD SYNC BANNER */}
                    <div className="bg-gradient-to-r from-emerald-500/10 via-teal-500/10 to-blue-500/10 p-4 border border-emerald-500/20 rounded-2xl flex flex-col sm:flex-row items-center justify-between gap-4 dark:from-emerald-950/20 dark:via-teal-950/20 dark:to-blue-950/20 dark:border-emerald-800/30">
                      <div className="flex items-center gap-3">
                        <div className="w-9 h-9 rounded-xl flex items-center justify-center bg-emerald-500 text-white animate-pulse text-sm shadow-sm flex-shrink-0">
                          ⚡
                        </div>
                        <div className="text-left">
                          <h4 className="text-xs font-black text-emerald-800 dark:text-emerald-400">ระบบซิงก์ข้อมูลคลาวด์แบบเรียลไทม์ (Cloud & Cross-Device Live Sync)</h4>
                          <p className="text-[11px] text-slate-500 dark:text-slate-400 font-medium">กดเลือกหัวข้อที่ต้องการตั้งค่าด้านล่างเพื่อเปิดหน้าต่างป๊อบอัพปรับแต่งได้อย่างสะดวกสบาย</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-2 bg-emerald-500/20 text-emerald-750 dark:text-emerald-400 px-3 py-1 rounded-full text-[10px] font-black flex-shrink-0">
                      ...
                      {SETTINGS_TOPICS.map((topic) => {
                        return (
                          <motion.button
                            ...
                            whileTap={{ scale: 0.98 }}
                            onClick={() => {
                              if (!tempSettings) setTempSettings(settings);
                              setActiveSettingsModalTopic(topic.id);
                            }}
                            className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700 rounded-3xl p-6 text-left shadow-sm hover:shadow-md transition-all flex flex-col justify-between group cursor-pointer relative overflow-hidden"
                          >
                            <div className="space-y-4">
                              <div className="flex items-center justify-between">
                                ...
                                >
                                  {topic.icon === 'Palette' && <Palette className="w-6 h-6" />}
                                  {topic.icon === 'Shield' && <Shield className="w-6 h-6" />}
                                  {topic.icon === 'Bell' && <Bell className="w-6 h-6" />}
                                  {topic.icon === 'FileText' && <FileText className="w-6 h-6" />}
                                  {topic.icon === 'Database' && <Database className="w-6 h-6" />}
                                  {topic.icon === 'Smartphone' && <Smartphone className="w-6 h-6" />}
                                </div>
                                {topic.badge && (
                                  <span 
                              ...
                              <div>
                                <h3 className="text-base font-black text-slate-850 dark:text-slate-100 group-hover:text-accent transition-colors">
                                  {topic.title}
                                </h3>
                            ...
                            <div className="pt-4 mt-4 border-t border-slate-100 dark:border-slate-800/80 flex items-center justify-between">
                              <span className="text-[11px] font-extrabold text-slate-400 group-hover:text-slate-600 dark:group-hover:text-slate-200 transition-colors">
                                {topic.category}
                              </span>
                              <div 
                                className="flex items-center gap-1 text-xs font-black transition-transform group-hover:translate-x-1"
                                style={{ color: (tempSettings || settings).colorAccent }}
                              >
                                <span>คลิกเพื่อตั้งค่า</span>
                                <span>→</span>
                              </div>
                            </div>
                          </motion.button>
                    ...
                    <SettingsModal
                      activeTopicId={activeSettingsModalTopic}
                      onClose={() => setActiveSettingsModalTopic(null)}
                      ...
                      handleAddMenuLink={handleAddMenuLink}
                      moveMenuLinkUp={moveMenuLinkUp}
                      moveMenuLinkDown={moveMenuLinkDown}
                      handleEditMenuLinkStart={handleEditMenuLinkStart}
                      handleRemoveMenuLink={handleRemoveMenuLink}
                      tasks={tasks}
                      expenses={expenses}
                      ...
                      adminCustomLinks={adminCustomLinks}
                      setShowInstallAppModal={setShowInstallAppModal}
                      presetCategoryFilter={presetCategoryFilter}
                      setPresetCategoryFilter={setPresetCategoryFilter}
                      applyThemePreset={applyThemePreset}
                      handleAccentColorChangeInput={handleAccentColorChangeInput}
                      harmoniousMode={harmoniousMode}
                      setHarmoniousMode={setHarmoniousMode}
                    />
